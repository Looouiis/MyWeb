...
export default{
    ...
    created(){
        if(this.default.indexOf('usr') !== -1){
            this.axios.get('http://localhost:801/users/comment/'+this.msgId).then((res) => {
            ...
            this.axios.get('http://localhost:801/anonymous/comment/'+this.msgId).then((res) => {
              if('status' in res.data && res.data.status){
                this.comList = res.data.content.comments
    ...
    methods:{
        ...
        submit(content){
            if(this.default.indexOf('usr') !== -1){
                ...
                url: 'http://localhost:801/users/comment/'+this.msgId
                }).then((res) => {
                console.log(res)
                if('status' in res.data && res.data.status){
                ...
                headers: { 'content-type': 'application/x-www-form-urlencoded' },
                data: 'content='+content,   // 用 qs 将js对象转换为字符串 'name=edward&age=25'
                url: 'http://localhost:801/anonymous/comment/'+this.msgId
                }).then((res) => {
                console.log(res)
                if('status' in res.data && res.data.status){
                    this.$message.success(res.data.description)
